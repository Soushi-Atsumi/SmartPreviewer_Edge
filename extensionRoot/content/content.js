...
function initializeVideo() {
	...
	document.addEventListener('fullscreenchange', () => {
		let edgeIsFullScreen = document.webkitFullscreenElement === undefined ? false : document.webkitFullscreenElement !== null && document.webkitFullscreenElement.id === floatingVideo.id;
	...
	floatingVideo.addEventListener('mouseout', (event) => {
		...
		if (!edgeIsFullScreen && floatingVideo.offsetHeight + floatingVideo.offsetTop <= event.clientY | floatingVideo.offsetLeft >= event.clientX | floatingVideo.offsetLeft + floatingVideo.offsetWidth <= event.clientX | floatingVideo.offsetTop >= event.clientY) {
			floatingVideo.height = floatingVideoDefaultHeight;
			floatingVideo.width = floatingVideoDefaultWidth;
			floatingVideo.pause();
			floatingVideo.removeAttribute('src');
...
function addPreviewers() {
	...
	//Video
	const videoExtensions = ['m3u8', 'mkv', 'mov', 'mp4', 'webm', 'wmv'];
			...
			hyperlinks[i].addEventListener('mouseenter', (event) => {
				if (!event.shiftKey) {
					...
					if (targetHref.match(regexPDF) === null) {
						...
						src = targetHref.match(regexPDF)[0];
					} else {
					...
					if (src !== '') {
						// In Edge, the size of iframe should not be changed after setting src, or it will not display the document correctly.
						setFloatingPDFSize(floatingPDFAspectRatio, floatingPDFAspectRatio);
						floatingPDF.setAttribute('src', src);
						setFloatingPDFFromMousePosition(latestMousePositionX, latestMousePositionY);
